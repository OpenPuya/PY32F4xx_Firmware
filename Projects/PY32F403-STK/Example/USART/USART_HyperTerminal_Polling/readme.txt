...
POLLING method. The USART is configured with a baud rate of 115200, 8 data bits, 
1 stop bit, and no parity. After compiling and downloading the program to the MCU, 
the host computer will receive data from 0x1 to 0xC through USART. Then, when the 
host computer sends 12 data bytes (0x1 to 0xC), the MCU will send back the received 
